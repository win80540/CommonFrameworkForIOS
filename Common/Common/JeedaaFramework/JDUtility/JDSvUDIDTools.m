...
//  SvUDID
//
//  Created by  Klueze on 8/18/13.
//  Copyright (c) 2013 maple. All rights reserved.
//

#import "JDSvUDIDTools.h"
...
static const char kKeyChainUDIDAccessGroup[] = "com.jeedaa.mini2s";//


@implementation JDSvUDIDTools

+ (NSString*)UDID
{
    NSString *udid = [JDSvUDIDTools getUDIDFromKeyChain];
    if (!udid) {
        ...
        if (version >= 7.0) {
            udid = [JDSvUDIDTools _UDID_iOS7];
        }
        else if (version >= 2.0) {
            udid = [JDSvUDIDTools _UDID_iOS6];
        }
        
        [JDSvUDIDTools settUDIDToKeyChain:udid];
    }
...
+ (NSString*)_UDID_iOS6
{
    return [JDSvUDIDTools getMacAddress];
}
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    if (queryErr == errSecItemNotFound) {
        DLog(@"KeyChain Item: %@ not found!!!", [NSString stringWithUTF8String:kKeychainUDIDItemIdentifier]);
    }
    else if (queryErr != errSecSuccess) {
        DLog(@"KeyChain Item query Error!!! Error code:%d", (int)queryErr);
    }
    if (queryErr == errSecSuccess) {
        DLog(@"KeyChain Item: %@", udidValue);
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([JDSvUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        [JDSvUDIDTools updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
        ...
        if (writeErr != errSecSuccess) {
            DLog(@"Add KeyChain Item Error!!! Error Code:%d", (int)writeErr);
            
            [dictForAdd release];
            return NO;
        }
        else {
            DLog(@"Add KeyChain Item Success!!!");
            [dictForAdd release];
            return YES;
        }
    }
...
+ (BOOL)removeUDIDFromKeyChain
{
    ...
    if (deleteErr != errSecSuccess) {
        DLog(@"delete UUID from KeyChain Error!!! Error code:%d", (int)deleteErr);
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    if (queryResult) {
        ...
        if (updateErr != errSecSuccess) {
            DLog(@"Update KeyChain Item Error!!! Error Code:%d", (int)updateErr);
            ...
            DLog(@"Update KeyChain Item Success!!!");
            [dictForQuery release];
            [dictForUpdate release];
            return YES;
        }
