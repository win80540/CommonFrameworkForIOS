...
- (id)initWithFrameImageItems:(CGRect)frame delegate:(id<SGFocusImageFrameDelegate>)delegate focusImageItems:(NSMutableArray *)imageItems
{
    self = [super initWithFrame:frame];
    if (self) {
        if (imageItems != nil && [imageItems count] > 0) {
            ...
            //va_start(argumentList, imageItems[0]);
...
- (void)setupViews
{
    ...
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.bounds.size.width *.5 - size.width *.5, self.bounds.size.height - size.height + 10, size.width, size.height)];
...
-(void)clickPageImage:(UIButton *)sender{
    NSArray *imageItems = objc_getAssociatedObject(self, (__bridge const void *)SG_FOCUS_ITEM_ASS_KEY);
    if (imageItems != nil && [imageItems count] >0) {
        SGFocusImageItem *item = [imageItems objectAtIndex:sender.tag];
